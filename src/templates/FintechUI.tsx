...
const StockGraph = () => {
  ...
  const pathD = points.reduce((acc, p, i) => i === 0 ? `M ${p.x} ${p.y}` : `${acc} C ${points[i - 1].x + (p.x - points[i - 1].x) / 3} ${points[i - 1].y}, ${p.x - (p.x - points[i - 1].x) / 3} ${p.y}, ${p.x} ${p.y}`, "");
  ...
  return (
    <svg width="100%" height="100%" viewBox="0 0 1000 200">
      ...
      <path d={`${pathD} L ${points[points.length - 1].x} 200 L ${points[0].x} 200 Z`} fill="url(#graphGradient)" />
